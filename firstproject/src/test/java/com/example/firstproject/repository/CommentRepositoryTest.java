...
class CommentRepositoryTest {
    ...
    void findByArticleId() {
        /* Case 1: 4번 게시글의 모든 댓글을 조회 */
        {
            ...
            List<Comment> expected = Arrays.asList(a, b, c); // 어레이 리스트 형식으로 묶어준다.


            // 검증
            assertEquals(expected.toString(), comments.toString(), "4번 댓글의 모든 댓글을 출력!");
        }
        /* Case 2: 1번 게시글의 모든 댓글을 조회 */
        {
            // 입력 데이터 준비
            Long articleId = 1L;
            ...
            List<Comment> expected = Arrays.asList(); // 어레이 리스트 형식으로 묶어준다.


            // 검증
            assertEquals(expected.toString(), comments.toString(), "1번 댓글은 댓글이 없음");
        }
    }
    ...
    void findByNickname() {
        /* Case 1: "Park"의 모든 댓글 조회 */
        {
            ...
            Comment c = new Comment(7L, new Article(6L, "당신의 취미는?", "댓글ㄱㄱㄱㄱ"),nickname, "조깅");
